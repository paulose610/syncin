...
class User(db.Model, UserMixin):
    ...
    cflags=db.Column(db.Integer(),default=0)
...
class Playlist(db.Model):
    ...
    song_id= db.Column(db.Integer(), db.ForeignKey('song.id', name='sid'), nullable=False)
...
class Cycledata(db.Model):
    ...
    id = db.Column(db.Integer(), primary_key=True)
    song_id = db.Column(db.Integer(), db.ForeignKey('song.id', name='sid'))
    likes = db.Column(db.Integer(),default=0,nullable=False)
    time_played = db.Column(db.Integer(), default=0, nullable=False)
    flags = db.Column(db.Integer(), default=0)
